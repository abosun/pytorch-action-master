...
if __name__ == '__main__':
    labels = get_label(testset)
    a = 0.0
    acc_max, a_max = 0, 0
    spa_score = np.load(args.spa)
    tim_score = np.load(args.tim)
    acc_spa = mean_class_accuracy(spa_score, labels)
    acc_tim = mean_class_accuracy(tim_score, labels)
#    acc_spa = test_with_score(spa_score)
#    acc_tim = test_with_score(tim_score)
    while a<100:
      all_score = a*spa_score + tim_score
      acc_all = mean_class_accuracy(all_score, labels)
