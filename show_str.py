...
dim = 2048
net = SpaNet(glo_channels=1280, loc_channels=768, out_channels=1, num_classes=class_num, drop_rate=drop_rate, out_type=args.type, n=args.n)
if use_cuda:
    net.cuda()
    net = torch.nn.DataParallel(net, device_ids=range(torch.cuda.device_count()))
    cudnn.benchmark = True
net.load_state_dict(torch.load('UCF101_split1_rgb_0.000_model_strall'))
...
def showall():
    ...
    for batch_idx, (inputs, targets) in enumerate(showloader):
        ...
        str_w = net(inputs+[tar_hot])
        str_w = str_w.data.cpu().numpy()
        #print(glo_w.size(), loc_w.size())
        pool = multiprocessing.Pool(processes = 16)
        for i in range(str_w.shape[0]//25):
            heat_str = str_w[25*i]
            np.savetxt('ucfstrweight/'+path_list[batch_idx*32+i]+'_str.txt' ,heat_str)
            path = path_list[batch_idx*BATCH_SIZE+i]
            ids = batch_idx*BATCH_SIZE+i
            #_fun_str(heat_str, path, ids)
            pool.apply_async(_fun_str, (heat_str, path, ids))
        pool.close()
        pool.join()
