...
dim = 2048
net = SpaNet(model_type='old', glo_channels=1280, loc_channels=8, out_channels=1, num_classes=class_num, drop_rate=drop_rate, out_type=args.type, n=args.n)
if use_cuda:
    net.cuda()
...
for epoch in range(start_epoch, start_epoch+epoch_num):
    ...
    train(epoch)
    train(epoch)
    torch.save(net.state_dict(), args.dataset+'_split'+args.split+'_rgb'+'_%.3f'%(best_acc)+'_model_oldall')
    break
