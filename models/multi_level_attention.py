...
class GloLocNet(nn.Module):
    ...
    def forward(self, x):
        ...
        glo = torch.split(glo,5,dim=0) if self.training else torch.split(glo,25,dim=0)
        ...
        loc = torch.split(loc,5,dim=0) if self.training else torch.split(loc,25,dim=0)
...
class SpaAttention_withW(nn.Module):
    def __init__(self, in_channels, out_channels, withW=False, **kwargs):
