...
def get_showing_img(image, w_glo, w_loc):
    ...
    res = np.concatenate([image, heat_glo, heat_loc], axis=1)
    return res
...
def _fun(heat_map_glo, heat_map_loc, path, ids):
    ...
    cv.imwrite('ucf101show_strold/'+path_list[ids]+'.jpg',res)
...
def showall():
    ...
    for batch_idx, (inputs, targets) in enumerate(showloader):
        ...
        for i in range(str_w.shape[0]//25):
            heat_str = str_w[25*i]
            heat_old = old_w[25*i]
            np.savetxt('ucfstroldweight/'+path_list[batch_idx*32+i]+'_str.txt' ,heat_str)
            np.savetxt('ucfstroldweight/'+path_list[batch_idx*32+i]+'_old.txt' ,heat_old)
