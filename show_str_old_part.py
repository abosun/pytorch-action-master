...
# print(sys.path)
from models.spa_struct_attention_show import *
import numpy as np
import datasets
...
str_net.load_state_dict(torch.load('UCF101_split1_rgb_0.000_model_strall'))
old_net.load_state_dict(torch.load('UCF101_split1_rgb_0.000_model_oldall'))

IMG_DIR = '/home/ss/lmy/temporal-segment-networks/data/UCF101_FLOW'
image_list = open(os.path.join(split_dir,'part.txt')).read().split('\n')
...
def get_showing_img(image, w_glo, w_loc):
    ...
    w_loc = pinghua(cv.resize(w_loc,(299,299)))
...
def get_showing_str(image, w_glo):
    ...
    w_glo = pinghua(cv.resize(w_glo,(299,299)))
    heat_glo = CAM(image, w_glo,0.5)
    res = np.concatenate([image, heat_glo], axis=1)
    return res

def _fun(heat_map_glo, heat_map_loc, path, ids):
    image_list = get_images(path)
    image = cv.imread(image_list[frame_id])
...
def _fun_str(heat_map, path, ids):
    ...
    heat_map = np.reshape(heat_map,(8,8))
    res = get_showing_str(image, heat_map)
    cv.imwrite('ucf101show_str/'+path_list[ids]+'.jpg',res)
    print( path_list[ids], 'has done')
...
def showall():
    ...
    correct = 0
    total = 0
    os.makedirs('ucf101show_strold'+'%02d'%(frame_id))
    os.makedirs('ucfstroldweight_'+'%02d'%(frame_id))
    for batch_idx, (inputs, targets) in enumerate(showloader):
        ...
        str_w = str_net(inputs+[tar_hot])
        str_w = str_w.data.cpu().numpy()
        old_w = old_net(inputs+[tar_hot])
        old_w = old_w.data.cpu().numpy()
        #print(glo_w.size(), loc_w.size())
        pool = multiprocessing.Pool(processes = 16)
        for i in range(str_w.shape[0]//25):
            heat_str = str_w[25*i+frame_id]
            heat_old = old_w[25*i+frame_id]
            np.savetxt('ucfstroldweight_'+'%02d'%(frame_id)+'/'+path_list[batch_idx*32+i]+'_str.txt' ,heat_str)
            np.savetxt('ucfstroldweight_'+'%02d'%(frame_id)+'/'+path_list[batch_idx*32+i]+'_old.txt' ,heat_old)
            path = path_list[batch_idx*BATCH_SIZE+i]
            ids = batch_idx*BATCH_SIZE+i
            pool.apply_async(_fun, (heat_old, heat_str, path, ids))
        pool.close()
        pool.join()
